...
Deploys and manages Nutanix Objects **object stores** and their TLS certificates on
Prism Central, wrapping `nutanix_object_store_v2` and
`nutanix_object_store_certificate_v2`.

Store level only — buckets have no Nutanix provider resource. S3 access keys _do_
exist, but via `nutanix_user_key_v2` (`key_type = "OBJECT_KEY"`), wrapped by
`tf-ntnx-iam` rather than here. See the [repository README](../README.md) for the
full capability boundary and the operational traps worth knowing before first use:
the 16-character name limit, the unconfirmed unit of `total_capacity_gib`, and the
fact that **update cannot change any field** — it only resumes a failed deployment,
so resizing means destroy and recreate.
